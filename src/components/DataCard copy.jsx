...
const DataCard = ({ id, title, typeLabel, typeValue, description, type }) => {
    ...
    return (
        <>
            <Card sx={{ borderRadius: 2, cursor: type === "use-case" ? "pointer" : "default", transition: "border-color 0.3s", boxShadow: 2, border: `1px solid ${isHovered ? '#286090' : '#D9D9D9'}` }}
                onMouseEnter={() => setIsHovered(true)}
                onMouseLeave={() => setIsHovered(false)}
                onClick={type === "use-case" ? useCase : undefined}
            >
                <Box sx={{
                    backgroundColor: isHovered ? '#286090' : '#e6f0fc', padding: '8px 16px', display: "flex",
                    alignItems: "center",
                    justifyContent: "space-between",
                    minHeight: '48px'
                }}>
                    <Typography variant='buttonText' sx={{ color: isHovered ? "#FFFFFF" : "#1F1F1F" }}>
                        {title}
                    </Typography>
